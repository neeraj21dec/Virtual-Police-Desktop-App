...
public class Challan extends JPanel implements ItemListener,ActionListener
{
JButton b1;
...
jt7=new JTextField(60);
jt8=new JTextField(60);
...
rs3=st3.executeQuery("select * from ctype");
while(rs3.next())
{
...
String father_name=jt4.getText();
...
String amount=jl15.getText();
String licno=jt7.getText();
String police_name=jt8.getText();
...
st6.executeUpdate("insert into challans values('"+name+"','"+mobno+"','"+address+"','"+father_name+"','"+vechicle_no+"','"+city+"','"+state+"','"+area+"','"+type+"','"+amount+"','"+licno+"','"+police_name+"','"+control_room+"','"+current_date+"')");
...
rs4=st4.executeQuery("select penalty from ctype where type='"+c3.getSelectedItem()+"'");
while(rs4.next())
{
 String penalty=rs4.getString(1);
  jl15.setText("Rs "+penalty);
