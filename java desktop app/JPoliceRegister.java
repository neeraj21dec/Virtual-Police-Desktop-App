...
class JPoliceRegister extends JPanel implements ItemListener,ActionListener
{
JButton b1,b2;
...
jl=new JLabel("Police Registration :-");
...
rs3=st3.executeQuery("select station from stations where city='"+city+"' and state='"+state+"'");
while(rs3.next())
{
...
String city=c1.getSelectedItem();
String state=c2.getSelectedItem();
String station=c3.getSelectedItem();
...
rs4=st4.executeQuery("select * from pprofile where mobno='"+mobno+"'");
...
{
st4.executeUpdate("insert into pprofile values('"+name+"','"+mobno+"','"+pwd+"','"+post+"','"+city+"','"+state+"','"+station+"','"+home+"')");
